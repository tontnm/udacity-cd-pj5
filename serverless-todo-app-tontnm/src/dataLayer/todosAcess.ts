...
export class TodosAccess {
  ...
  async getTodoItems(userId: string): Promise<TodoItem[]> {
    logger.info(`getTodoItems for user ${userId} from ${this.todosTable}`)
  ...
  async getTodoItem(userID: string, todoId: string): Promise<TodoItem> {
    logger.info(`getTodoItem ${todoId} from ${this.todosTable}`)
  ...
  async createTodoItem(todoItem: TodoItem) {
    logger.info(`createTodoItem ${todoItem.todoId} into ${this.todosTable}`)
  ...
  async updateTodoItem(userId: string, todoId: string, todoUpdate: TodoUpdate) {
    logger.info(`updateTodoItem ${todoId} in ${this.todosTable}`)
  ...
  async deleteTodoItem(userId: string, todoId: string) {
    logger.info(`deleteTodoItem ${todoId} from ${this.todosTable}`)
  ...
  async updateAttachmentUrl(userId: string, todoId: string, attachmentUrl: string) {
    logger.info(`updateAttachmentUrl for todo ${todoId} in ${this.todosTable}`)
